    ...
    io.on("connection", (socket) => {
        ...
        socket.on("join_individual_room", function(room){
            ...
            if(a==socket.decoded_token.sub){
                console.log("somebody join INDIVIDUAL room", room,'with user_id',socket.decoded_token.sub);
                socket.join(room)
            }else{
        ...
        socket.on("message", ({conversation, data }) => {
            ...
            console.log('send message to conversation_id ',conversation.id,':', data);
            // let receivers = [];
            ...
            const conversation_to_receiver = {...conversation,users:[{...data.user}]}
            conversation.users.forEach(user=>{
               
                const individual_room = "conversation_list_"+user.id;
                io.to(individual_room).emit("conversation", {conversation:conversation_to_receiver,payload:payload});
            });
            // io.to().emit()
            ...
            const postData = {conversation_id:conversation_id, value:data.message.value} 
            require('../socket/saveMessage')({postData:postData,jwt:socket.jwt});
        ...
        socket.on("stopped_typing", ({ conversation, sender }) => {
            console.log(sender, 'is STOPPED typing in conversation_id',conversation.id);
            // mengirim emit ke semua user participant di conversation_id
            conversation.users.forEach(user=>{
                const receiver_room = 'conversation_list_'+user.id;
